...
public class HXHdlcDLMS implements IProtocol {
	 ...
	 * @return
	 */
	private boolean LinkNoAuth(HXFramePara fpara, ICommucation commDevice) {
		byte[] sndByt = null;
		byte[] receiveByt = null;
		boolean isSend = false;
		Nrec = 0;
	 ...
	 * @return
	 */
	private boolean LinkLLSAuth(HXFramePara fpara, ICommucation commDevice) {
		byte[] sndByt = null;
		byte[] receiveByt = null;
		boolean isSend = false;
		Nrec = 0;
	 ...
	 * @return
	 */
	private boolean LinkHLSAuth(HXFramePara fpara, ICommucation commDevice) {
		byte[] sndByt = null;
		byte[] receiveByt = null;
		boolean isSend = false;
		Nrec = 0;
	...
	@Override
	public byte[] Read(HXFramePara paraModel, ICommucation commDevice) {
//		//光电只握手一次，帧序号，不可清零
...
//		}
		...
		if (paraModel.FirstFrame) {
			...
			Nrec++;
			paraModel.Nrec = Nrec;
			if ((receiveByt[11 + paraModel.DestAddr.length] & 0xff) == 0x01) {
				for (int i = 14 + paraModel.DestAddr.length; i < receiveByt.length - 3; i++) {
					rtnReceiveByt.add(receiveByt[i]);
				}
			}
			if ((receiveByt[11 + paraModel.DestAddr.length] & 0xff) == 0x02) {
	...
	@Override
	public boolean DiscFrame(ICommucation commDevice) {
		byte[] Recdata;
